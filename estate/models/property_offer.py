...
class PropertyOffer(models.Model):
    ...
    type_id = fields.Many2one(
        related="property_id.type_id", string="Property Type", readonly=True
    )
        ...
        for vals in vals_list:
            ...
            if (
                ...
                * self.env["estate.property"].browse(vals["property_id"]).expected_price
                ...
                raise UserError(
                    "You are expected to make an offer greater than 90% of the expected price"
                )
            for offer_id in (
                self.env["estate.property"].browse(vals["property_id"]).offer_ids
            ):
                if offer_id.price >= vals["price"]:
                    raise UserError(
                        "You are expected to make a higher offer than those already entered"
                    )
        ...
        for record in self:
            try:
                record.date_deadline = fields.Date.add(
                    value=record.create_date, days=record.validity
                )
            except:
                record.date_deadline = fields.Date.add(
                    value=fields.Date.today(), days=record.validity
                )
